...
_mergeSort:
	...
	cmp		rdx, rsi			; if (right > left)
	jg 		middle 	
	jle 	end

middle:
	; Find middle point to divide the array in two
	; mid = (left + right) / 2
	mov 	r13, rsi		; store left param into m
	add 	r13, rdx		; add right param to left
	sub 	r13, 1			; mid - 1
	shr 	r13, 1			; divide by 2
...
mergeLeft:
	...
	push 	rdx
	push 	r13
	mov 	rdx, r13		; replace right with mid
	call 	_mergeSort 		; call mergeSort
	pop		r13
	pop 	rdx
	pop 	rsi
...
mergeRight:
	...
	push 	rdx
	push 	r13
	add 	r13, 1			; add one to r13 (mid + 1)
	mov 	rsi, r13		; replace left with mid + 1
	call 	_mergeSort 		; call mergeSort
	pop		r13
	pop 	rdx
	pop 	rsi
